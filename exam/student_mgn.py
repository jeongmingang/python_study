...
def add_std_info():
    ...
    temp.extend(get_std_info('성명 국어 영어 수학을 입력하세요 ex)최영민 90 90 90 >>'))
    if len(temp) == 5:
...
def update_std_info():
    up_num = input("수정할 학생번호를 입력하세요. >>")
    for std in std_list:
        if std[0] == up_num:
            temp = get_std_info('수정할 국어 영어 수학을 입력하세요. ex)90 90 90 >>')
            if len(temp) == 3:
                del std[2:5]
                std.extend(temp)
                return
...
def delete_std_info():
    del_num = input("삭제할 학생 번호를 입력하세요. >>")
    for std in std_list:
        if std[0] == del_num:
            std_list.remove(std)
            return
